...
- **Chunk S2 — split into S2a (additive) + S2b (cutover); gate flip deferred to S5.**
  ...
  **`success_url` vs webhook race (FOLLOW_UPS #84): ✅ RESOLVED 2026-07-12.**
  The real `success_url` lands on `/app/voice/processing` before
  `checkout.session.completed` writes the trial — a just-paid `none` user would be
  bounced back to Card Capture (and would 402 at `/start`). Fixed by
  reconcile-on-landing: `reconcileCheckoutSession` retrieves the session and
  writes the row synchronously before the guard decides. The flag flip is no
  longer gated on this. See `docs/session-checkout-race/`.
