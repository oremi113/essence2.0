...
| 2026-07-17 triage | 8×P3 | **8 new per-file items** — DB spend-caps bypassable under concurrency; `loadReadyVoiceProfile` DB-error swallow wedges the paid flow; `envInt` treats a cap of 0 as "use default" (defeats the spend kill-switch); Stripe portal `returnPath` backslash open-redirect; ElevenLabs client-abort orphans a billed voice; duplicate voice-profile race (owner-paired); no tests on the voice spend-gating logic; `audio/commit` downloads the whole clip just to read its size. See [`docs/follow-ups/INDEX.md`](follow-ups/INDEX.md). | 📁 per-file |
