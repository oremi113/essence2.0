...
- **Verified 2026-06-19 for real (colima):** ran the exact CI leg locally under a Docker daemon — `supabase db start` + `migration up --local` applied all 24 migrations to a fresh Postgres cleanly, and `check:types` is green. Earlier I had argued equivalence from `db push --dry-run` = "Remote database is up to date"; **that was unsound** — `db push --dry-run` only compares the migration *ledger*, not the actual schema. The real from-migrations build surfaced genuine drift the committed (remote-generated) `types.ts` was hiding, reconciled below.
- **Drift the check caught + fixed (migration `20260619170000_capture_orphaned_remote_objects.sql`):** the remote DB carried objects no migration created — most importantly `public.healthcheck()`, which is **used in production** by `/api/health` (`supabase.rpc("healthcheck")`); a DB rebuilt from migrations lacked it, so `/api/health` would have failed on any fresh environment. Also two unused `training_clip_status` enum values (`pending_upload`, `missing`). All three are now captured as a migration (definitions pulled verbatim from remote), so migrations reproduce remote. The committed `types.ts` was regenerated from `--local` as the canonical artifact (net change: the optional `__InternalSupabase` metadata block drops out — harmless; `tsc` green). The new migration is idempotent (`create or replace`, `add value if not exists`), so a future `db push` to remote — where the objects already exist — is a no-op.
...
Two parallel read-only bug-hunt agents swept the post-2026-06-13 surfaces (Step 8 Home B, the a11y contrast pass, Stripe lifecycle hardening, onboarding) plus the Step 6 message routes. Health at scan time: typecheck ✅ · lint ✅ · 313/313 unit ✅. The Stripe money path, audio/voice pipeline, Home B, and the a11y pass all swept genuinely clean. The four findings below are all the same root-cause class the 2026-06-13 pass surfaced — an unchecked Supabase write (or unchecked affected-rows) that returns success regardless — clustered in the Step 6 message routes, the one corner the prior pass treated as WIP. **The durable fix for this whole class is a lint rule / checked-write wrapper, not the Nth point fix** — flagged here, not yet built.

### 61. [P3] generate/regenerate success-mark writes ignore their `{ error }`
`src/app/api/messages/generate/route.ts:301-305`, `src/app/api/messages/regenerate/route.ts:257-261` and `:162-170`, `src/lib/messages/audio.ts:87-91` — the writes that record a successful text/audio render into `pending_generations` discard `{ error }`, then the route returns `{ textStatus: "succeeded", audioStatus: "succeeded" }` regardless of whether the write landed.
**Why it matters:** lower impact than #62 because no cost-counter is involved and the next action (`/save`, `/commit`) re-reads `audio_status`/`generated_text` and 409s ("not ready to save yet") if the mark was lost — so the divergence is caught downstream, not corrupted into a saved message. But the client is told "succeeded" while the server may say `pending`, producing a "ready" UI that fails on save, and the audio object is orphaned in storage if only the final mark fails.
**Fix shape:** check `{ error }` on these success-mark updates and return the same `audioStatus: "failed"`/retryable shape the failure branches already use; `audio.ts:87` should return `{ ok: false }` when the final mark fails rather than `{ ok: true }`.
**Pick up when:** next pass touching the Step 6 generation pipeline. Agent-fixable.

### 62. [P2] ✅ RESOLVED (fix/step6-unchecked-writes, 2026-06-19) — commit render-cap silently bypassable on a dropped promote write
**Resolution:** `src/app/api/messages/commit/route.ts` now captures `{ error }` on the candidate→committed promotion update (which also bumps `audio_render_count`) and returns a 502 retryable on failure instead of unconditionally returning `committed: true`. Mirrors the existing TTS/upload failure branches directly above it; the deterministic upload path makes the retry idempotent. Original finding below.

After a *successful, paid* ElevenLabs render + storage upload, the final `update()` (`route.ts:116`) that promotes `candidate_text → generated_text`, sets `audio_path`/`audio_status`, and bumps `audio_render_count` discarded its `{ error }` and the route returned `{ committed: true }` regardless. `defineRoute`'s try/catch doesn't help — Supabase reports write errors via the returned `error`, it doesn't throw.
**Why it mattered:** a silently-failed write left `audio_render_count` un-bumped while the audio object was already uploaded (`upsert: true`). `audio_render_count` is the cost-cap counter (checked at `:66` against `maxAudioRenders`), so each dropped bump granted a free extra *paid* render — and the DB text diverged from the audio the user now hears.

### 63. [P4] onboarding-completion UPDATE treats "zero rows matched" as success
`src/lib/onboarding/persistOnboardingCompletion.ts:34-42` (caller `src/app/onboarding/page.tsx:128`) — the write is `.update({...}).eq("user_id", userId)` and inspects only `{ error }`. PostgREST returns `error: null` when the predicate matches **zero rows**. FU #42 closed the *DB-error* door (a real error now throws), but a no-op update (profile row absent / RLS-hidden from this client) still resolves as success — the same lost-onboarding end-state #42 set out to kill.
**Why it's Low / near-harmless today:** the `on_auth_user_created` trigger creates the `profiles` row at signup, so the `eq("user_id")` predicate realistically always matches; the guarantee is implicit, not enforced at this write.
**Fix shape:** append `.select("user_id")` to the update and throw when the returned set is empty (zero rows affected), so a no-op write is treated as a failure rather than a silent save. Mirrors #42's loud-failure intent.
**Pick up when:** any onboarding-persistence pass. Agent-fixable.

### 64. [P3] ✅ RESOLVED (fix/step6-unchecked-writes, 2026-06-19) — `generations/[id]/play` was the 4th signed-URL route #45's fix missed
**Resolution:** `src/app/api/messages/generations/[id]/play/route.ts` now records `signed_url_playback` usage *after* `createPlaybackSignedUrl` succeeds, not at the top of the handler — exactly the move `5fef4ea` made to the other three signed-URL routes (init-upload, playback-url, messages/[id]/play). That commit's sweep covered three of four routes; this pre-save preview route (added later for the A6 take-preview) was outside it, so it kept logging "success" on 404/409/400/sign-failure and consuming the per-minute signed-URL budget on requests that issued no URL. Same blast radius #45 described, now closed across all four routes.
