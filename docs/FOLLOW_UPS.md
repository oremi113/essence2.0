...
### 76. [P3] Legal pages (`/privacy`, `/terms`) are absent from `main` — not merely unlinked
**Refined 2026-08-25 (triage) — the premise below is now false on `main`.** The Privacy/Terms pages this entry assumed "shipped greenfield" **do not exist on the current default branch**: there is no `src/app/privacy` / `src/app/terms`, no `src/components/screens/legal/`, no `/dev/{privacy,terms}`, and `ROUTES` has no `privacy` / `terms` constant (grep-confirmed against `main` @ 93d0bbd). The greenfield branch (task C) was evidently never merged. So this is no longer "build the link" — it's "the pages themselves aren't in production." Step 9 Settings **has** since merged (`src/app/app/settings/`, `ROUTES.settings`), so the intended host for the links exists; the blocker moved from Settings to the pages. `docs/MASTER_SPEC.md:125` defers legal flows to v2 "unless legally required at launch," but app-store / platform review generally expects reachable Privacy + Terms, so this stays a launch-relevant gap. Owner/legal-paired (content + the merge decision). *Original entry (premise now stale) below.*
