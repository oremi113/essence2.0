...
| 59 | P3 | Legacy message-creation API orphaned by M0 — `POST /api/messages` + status-poll `GET /api/messages/:id` unreachable; POST still spends ElevenLabs with no Step 6 cost cap *(new 2026-06-19)* | ⚠️ PARTIAL — `POST /api/messages` deleted (owner-approved URL removal 2026-06-30); status-poll `GET /api/messages/:id` deletion still deferred |
...
| 81 | P2 | Account-delete teardown drops the subscriptions read's error → a transient DB hiccup deletes the account while its Stripe subscription keeps billing *(triage 2026-07-10)* | 🔶 owner-paired (Stripe/auth teardown); mechanically a one-line check |
| 82 | P2 | `retry_audio` renders paid ElevenLabs audio with NO cost cap, hourly gate, or ledger — an authenticated loop = unbounded vendor spend *(triage 2026-07-10)* | 🔶 owner-paired (cost-control surface); add cap + ledger like the control arm |
| 83 | P2 | A failed message generation permanently wedges creation — the orphaned active pending row 429s every retry via `pending_max`, forever *(triage 2026-07-10)* | ✅ agent-fixable — discard/supersede the failed row, or retry via `/regenerate` |
| 84 | P3 | Single-clip playback retry guard resets itself every fetch → an undecodable training clip bursts the rate-limited playback-url endpoint *(triage 2026-07-10)* | ✅ agent-fixable — stop resetting the ref inside `loadPlaybackUrl` |
| 85 | P3 | Account-delete storage wipe caps each prefix at 1000 objects with no pagination → a heavy user's audio survives "your voice has been erased" *(triage 2026-07-10)* | 🔶 delete-path; paginate the `list()` loop |
| 86 | P3 | No unit tests on `RecordScreen.reducer` + `useSequenceTimeline` — central shipping logic + a reused timer primitive, both uncovered *(triage 2026-07-10)* | ✅ agent-fixable — add reducer + fake-timer tests |
| 87 | P3 | The TTS→upload→duration→status-write pipeline is implemented twice (`audio.ts` vs `commit/route.ts`) and will drift on a vendor-spend path *(triage 2026-07-10)* | ✅ agent-fixable — extract one shared render-upload-duration helper |
| 88 | P4 | Onboarding draft-save persists the expiring `avatarUrl` signed URL to localStorage, violating the module's own "never persisted" contract *(triage 2026-07-10)* | ✅ agent-fixable — strip `avatarUrl` before `saveDraft` |
...
### 59. [P3] ⚠️ PARTIAL (2026-06-30) — Legacy message-creation API orphaned by M0 — `POST /api/messages` + status-poll `GET /api/messages/:id` are mounted but unreachable
**Resolution (the money-adjacent half):** `POST /api/messages` — the landmine — is **deleted** (owner-approved URL removal 2026-06-30). `src/app/api/messages/route.ts` is now GET-only (the live Memory Shelf list, kept); the dead handler and its POST-only imports are gone, the orphaned `messageCreateSchema` + `MessageCreateBody` removed from `src/lib/api/schemas.ts`, and the matching `tests/unit/schemas.test.ts` block dropped (full unit suite 329 green, tsc + lint clean). The vendor-spending, cost-cap-less endpoint can no longer be wired up by accident.
**Still deferred (owner choice 2026-06-30):** the status-poll `GET /api/messages/:id` (`src/app/api/messages/[id]/route.ts`) remains mounted — it spends nothing and is harmless, so its URL removal was held back. Pick it up in a later message-API consolidation pass if/when wanted. Original entry below.
...
### 74. [P3] C3 Vault Limit port must reuse `src/lib/vault-render/`, not the prototype's inlined engine fork
The approved C3 design prototype (`prototypes/essence-c3-vault-limit.html` / delivered `~/Downloads/essence-c3-vault-limit.html`) inlines a **verbatim, single-`t`** copy of the vault canvas engine + `RELIQUARY` palette from `prototypes/vault-canvas-rig.html`. Inlining is correct *for a self-contained prototype*, but the production truth has since diverged: `src/lib/vault-render/{vaultEngine,paintVault,palette}.ts` moved to a split-drive API (`VaultDrive = number | {mechT, emberT}`, `paintVaultFrame(canvas, drive, opts)`). Porting the inline verbatim would create a **third divergent engine copy**. Two subtler traps ride along:
- **Invented tokens.** `--shimmer-alpha` / `--shimmer-r-rest` (proto `:root`) are **not in `@theme`**; the canonical shimmer primitive is `--shimmer-intensity` (opacity-driven, single value). At port, `getComputedStyle` for the invented names → `NaN` → the `readAnchors()` bridge silently falls back to literals `0.06`/`0.52` — the Step-7 "defined-but-unused" trap the prototype header claims to guard against.
- **Format mismatch.** `--color-glow-warm-rgb` is **comma form** `214, 162, 92` in `@theme`; the prototype declares space form and parses with `split(/\s+/)` → `[NaN, NaN, 92]` → silent fallback to the palette literal. `palette.ts` deliberately avoids CSS-string parsing (array + mirror comment) — that's the established pattern.
...
## Discovery pass (triage 2026-07-10)

Read-only discovery run per `docs/DISCOVERY_AGENT.md`. Health at scan time on `main` (`1983bb9`): typecheck ✅ · lint ✅ · unit tests **370/370** ✅. This pass deep-read the subsystems that shipped since the last triage but were never read at depth: the **account-deletion teardown** (Step 9), the **Step 3 vault-render / card-capture** canvas engine, the **Step 6 message-generation routes** (cost-control + failure paths), the **Step 10 gen-failure / offline** surfaces, and the **record / onboarding** shared libs. Active WIP branches `feat/legal-pages` and `feat/first-breath-audio` (both last touched 2026-07-08) were treated as work-in-progress and **excluded** — the First Breath audio TODOs (FU-41) and legal-page wiring (FU-76) live there. The freshly-merged Stripe lifecycle audit (#92 → FU-77/78/79) and S10-B offline (#89 → FU-80) were **not** re-scanned (already covered). Eight new items logged (cap reached); one further P4 noted below but not logged.

**Reconciliation note for the fixer:** on `main`, `src/lib/messages/audio.ts:97-119` now **checks** its audio success-mark write (`markError` → returns failure). That is one of the three sites FU-66 flagged as unchecked. Discovery does not strike — recommend the fixer verify the FU-66 sites against current `main` and strike the resolved half.

### 81. [P2 · owner-paired] Account-delete teardown drops the `subscriptions` read error → a deleted account can keep being billed
`src/app/app/settings/actions.ts:191` — `deleteAccountAction` step 1 does `const { data: subs } = await service.from('subscriptions').select('stripe_subscription_id, status').eq('user_id', userId)` and **ignores `{ error }`**. A Supabase `.select()` that errors returns `{ data: null, error }` (it does not throw, so the surrounding `try/catch` never sees it). On a transient read failure `subs` is null, `for (const sub of subs ?? [])` iterates zero times, **no Stripe subscription is cancelled**, and the teardown proceeds to delete every row and the auth user anyway.
**Why it matters:** the step-1 comment (l.159-162) explicitly promises "cancel any live Stripe subscription so a deleted account is never billed … a hard failure aborts BEFORE any data loss." An unchecked *read* defeats that invariant: a momentary DB hiccup during account deletion leaves a live subscription charging the user's card for an account that no longer exists — and because the `subscriptions` row is cascade-deleted with the auth user, the `stripe_subscription_id` is gone, so it can't be found and cancelled later. Money + trust + a stated safety invariant, all on the irreversible delete path.
**Fix shape:** destructure `{ error }` on the read and abort into the "we couldn't finish closing your account" failure terminal (exactly like the storage/auth steps do) before any data loss. Owner-paired because it sits in the Stripe/auth teardown surface — the code change is a one-line error check, but confirm the abort copy + telemetry with the owner.
**Pick up when:** before launch (account deletion is a shipping, irreversible flow), or the next Stripe/settings pass. Pairs with FU-77/78 as billing-hardening.

### 82. [P2 · owner-paired] `retry_audio` renders paid ElevenLabs audio with no cost cap, hourly gate, or ledger entry
`src/app/api/messages/regenerate/route.ts:94-135` — the `mode: "retry_audio"` branch checks only that `generated_text` exists, resets `audio_status` to pending, and calls `generateAndStoreAudio` (a paid ElevenLabs render). Unlike the control arm 80 lines below it (`nextCount > maxRegenerates` → `regenerate_cap`, and `countGenerationsThisHour >= maxGenerationsPerHour` → `hourly_max`, plus a `recordUsageEvent` ledger row at l.225), `retry_audio` has **none** of these — no per-generation attempt cap, no hourly backstop, no usage ledger, and no precondition that the prior audio actually failed (it resets status regardless).
**Why it matters:** the endpoint is a plain authenticated `POST`. Any signed-in client (or a script with a session cookie) can call `retry_audio` on one existing generation in a loop and rack up unbounded paid ElevenLabs renders — evading even the 20/hr `hourly_max` global backstop that guards every other generate path. This is the exact "unmetered ElevenLabs invocation" cost-exposure the P1/P2 band exists for; it isn't firing in the human-paced happy path (the A6 retry button is one tap), which is why it's a landmine rather than an active leak.
**Fix shape:** before the `generateAndStoreAudio` call, record a `started` usage event and check `countGenerationsThisHour(...) >= maxGenerationsPerHour` (reuse the control-arm gate), and/or count `retry_audio` renders against a per-generation attempt cap; require `audio_status !== 'succeeded'` so a succeeded render can't be re-billed. Owner-paired (cost-control / vendor-spend surface).
**Pick up when:** before launch, or the next Step 6 cost-control pass. Direct sibling of FU-22's cost-exposure lineage.

### 83. [P2] A failed message generation permanently wedges creation — the orphaned pending row 429s every retry
`src/app/api/messages/generate/route.ts` (cold-start gate `:184`, row insert `:237`, failure returns `:287/:323/:357`) + `src/app/messages/new/MessagesNewPageClient.tsx:76-92` (the A5 "Try again" handler). Cold-start inserts the `pending_generations` row *before* text/audio run; if text or audio then fails, the route returns 502 but the row stays **active** — `saved_message_id` and `superseded_at` are both still null, which is exactly what `countActivePending` counts (`src/lib/messages/cost-controls.ts:102-111`). `superseded_at` is only ever set on an edit-note *success* (`:372`), and no sweep/expiry clears failed rows (grep-confirmed). The A5 retry re-POSTs a fresh **cold-start** `/generate`, which hits `countActivePending >= maxActivePendingPerUser (1)` → `pending_max` 429.
**Why it matters:** one transient LLM/TTS failure on the core message-creation flow leaves an orphan active row that (a) makes the A5 "Try again" button fail with `pending_max` **forever** — the retry can never succeed — and (b) blocks **every future message** the user tries to create, since the cap is one active flow per user. The 3-attempt ceiling is also reached via guaranteed-failing retries rather than real ones. A user-visible dead-end on a shipping main flow.
**Fix shape:** on a failed generation, discard/supersede the failed pending row (stamp `superseded_at` or delete it) so it stops counting; or have the A5 retry route through `/regenerate` (`retry_audio` / variant) against the existing `generationId` instead of re-POSTing a cold-start. Agent-fixable; the smallest correct fix is to release the `pending_max` slot the moment a generation terminally fails.
**Pick up when:** soon — shipping-path user lockout. Next Step 6 spine touch.

### 84. [P3] Single-clip playback retry guard resets itself on every fetch → an undecodable clip bursts the rate-limited playback endpoint
`src/components/audio/RecordingUpload.tsx:263` — `loadPlaybackUrl` sets `playbackRetriedRef.current = false` on every successful fetch (the "allow one retry" line). But `handleAudioError` (`:277-286`) also calls `loadPlaybackUrl` on its one-retry path. So for a clip that persistently fails to *decode* (a corrupt/truncated upload, an unsupported codec — not merely an expired URL), the cycle is: audio errors → `handleAudioError` sets the ref true and calls `loadPlaybackUrl` → fetch succeeds → **ref reset to false** → new URL set → audio errors again → ref is false again → retry … The sibling list-playback flow (`playClip`, using `playbackRetryUsedRef`, `:288-307`) is correctly bounded to one retry, so the two paths have drifted.
**Why it matters:** the "one retry" guard on the single-clip player doesn't actually bound anything — an undecodable training clip fires a rapid burst of `POST /api/audio/playback-url` calls (the audio element re-errors immediately on each new URL) until the endpoint's rate limiter starts 429ing. It self-throttles eventually and only spends the user's own budget, but it's console-spam, wasted battery/requests, and a latent correctness drift from the correctly-bounded sibling.
**Fix shape:** stop resetting `playbackRetriedRef` inside `loadPlaybackUrl`; reset it only when a new clip is selected (`resetToRecordAgain` already does) or after a clean, error-free play — mirror how `playClip`/`playbackRetryUsedRef` bounds the list path.
**Pick up when:** next time the record/playback UI is touched. Agent-fixable.

### 85. [P3 · delete-path] Account-delete storage wipe caps each prefix at 1000 objects with no pagination
`src/app/app/settings/actions.ts:263` (called from the teardown at `:217`) — `listAllStorageObjects` does `service.storage.from(bucket).list(prefix, { limit: 1000 })` and recurses per sub-folder, but never paginates within a prefix: Supabase Storage `list` returns at most `limit` entries per call, and there is no `offset`/continuation loop. If any single prefix holds more than 1000 objects, only the first 1000 are collected and removed; the auth user is then deleted regardless.
**Why it matters:** the delete terminal tells the user "your voice … [has] been erased," and the flow is a privacy-sensitive "delete my account." A heavy long-term user (many saved-message renders + pending audio accumulating under one `users/{userId}/…` prefix) could exceed 1000 objects, leaving real voice recordings orphaned in storage after the account and its DB rows are gone — contradicting the erasure promise, with no row left to trace them by. Low frequency at MVP volumes, but it's a data-hygiene / privacy correctness gap on an irreversible flow.
**Fix shape:** loop `list` with an incrementing `offset` (or a sort+cursor) until a short page returns, accumulating all paths before the `remove`. Keeps the existing recursive-per-prefix structure.
**Pick up when:** before launch (privacy/erasure correctness), or the next settings/storage pass. Pairs with #81 (both on the delete teardown).

### 86. [P3] No unit tests on `RecordScreen.reducer` or `useSequenceTimeline`
`src/components/screens/RecordScreen.reducer.ts` (`recordReducer` / `deriveInitialView`) and `src/lib/animation/useSequenceTimeline.ts` ship with no unit coverage. The reducer is a pure state machine driving the entire record flow's stage / celebration / working→ready branching; the timeline hook is a reusable timer primitive with non-trivial reset / skipTo / paused / strict-mode-guard behavior (touched only incidentally by one e2e spec).
**Why it matters:** these are exactly the testable units the repo otherwise covers (cf. `tests/unit/preview-refine-reducer.test.ts`), and they sit on a shipping path (record + every screen that sequences beats). A regression in a transition or a timer-cleanup path would ship silently — the house "extract, then test" pattern was half-applied (extracted, never tested).
**Fix shape:** add `tests/unit/record-reducer.test.ts` covering each transition + the celebration `next.kind` branches, and a fake-timer test for `useSequenceTimeline` (reset / skipTo / pause cleanup, StrictMode double-invoke guard).
**Pick up when:** next time either file is touched, or a test-coverage pass. Agent-fixable (pure logic + fake timers, no browser).

### 87. [P3] The TTS→upload→duration→status-write pipeline is implemented twice and will drift
`src/lib/messages/audio.ts:68-121` (`generateAndStoreAudio`, the generate/retry pending-flip caller) and `src/app/api/messages/commit/route.ts:76-128` (the deferred-audio candidate-promotion path) both implement the same core: render via ElevenLabs → upload to `pendingGenerationAudioPath` → derive duration via `mp3DurationMsFromByteLength` → write `audio_path` / `audio_status` / `audio_duration_ms`. The two copies already have subtly different post-conditions (e.g. their success-mark error handling), so a future change to the storage path, the duration calc, or the upload options must be made in both places or they silently diverge.
**Why it matters:** duplication on a vendor-spend path is the kind that drifts out of sync — a fix or a cost-control tweak applied to one renderer and not the other reintroduces a bug on the other flow (this is how FU-27's per-category-voice-settings gap originally spread across render paths).
**Fix shape:** extract the shared render → upload → duration core into one helper (e.g. in `audio.ts`) that both the pending-flip caller and commit's candidate promotion reuse; keep each caller's distinct pre/post bookkeeping local.
**Pick up when:** next Step 6 audio-pipeline touch, or an API-consolidation pass. Agent-fixable, pure refactor with the smoke suite as the guard.

### 88. [P4] Onboarding draft-save persists the expiring `avatarUrl` despite the module's "never persisted" contract
`src/components/screens/onboarding/state.ts:153` — the draft-persist effect calls `saveDraft({ currentScreen, form })`, and `form` includes `avatarUrl` (a signed URL that expires after ~1 hour). The module's own contract (l.13-15: "avatarUrl is NEVER persisted: signed URLs expire … the server is the source of truth") and the load side (l.144 skips `avatarUrl` on hydration) both assume it is never written — but the write side serializes the whole `form`, so the expiring URL *is* stored in `localStorage` on every field change.
**Why it matters:** low impact (the stored URL is skipped on load, expires quickly, is scoped to the user's own avatar, and lives only in that user's browser) — but it's a documented-contract violation that leaves a stale, expiring credential sitting as dead data in browser storage, and it will mislead the next reader who trusts the "never persisted" comment.
**Fix shape:** strip `avatarUrl` before persisting, e.g. `saveDraft({ currentScreen, form: { ...form, avatarUrl: null } })`. One line; honours the existing contract and the load-side skip.
**Pick up when:** next onboarding-state touch, or any draft-persistence hardening. Agent-fixable.

### §72 sub-note — two latent `useShimmerLoop` bugs to fix when the real Processing clock lands (refines FU-72)
Surfaced reading the Step 3 vault subsystem; **not** logged as standalone entries because they're inert until FU-72's real generation-polling clock exists (in `/dev/processing` every mock state mounts fresh on the "instant" path, so no transition runs). Recorded here so whoever builds the clock fixes them in the same pass:
- **Shimmer transitions snap instead of tween.** `src/components/screens/step3/useShimmerLoop.ts:143` writes `write(eng.base * mul)`, but the eased per-frame value is computed into the local `base` at `:121` and `eng.base` is only updated when the tween *finishes* (`:125/:134`). So during both the faint→active climb and the active→neutral exit, the shimmer holds at its start value for the whole duration then jumps to the end value in one frame — the "eases active → neutral" handoff the Reveal is meant to build from is a hard cut. Fix: `write(base * mul)`.
- **Dead `: ACTIVE` branch.** `:175` `const from = prev > NEUTRAL ? prev : ACTIVE;` sits inside `if (!instant && prev > NEUTRAL)`, so the `: ACTIVE` arm is unreachable — `from` is always `prev`. It reads as a meaningful fallback that can never fire, misleading the next person tuning the exit start. Fix: `const from = prev;`.

*One more lower-priority item found, not logged this run:* the vault **restore** "restart" path (`src/app/app/vault/restore/actions.tsx:52`) hand-rolls the checkout POST inline instead of routing through the shared `useCheckout` hook that Seal/Protect use — so a mock/internal checkout URL is handled with an unconditional `window.location.href` rather than `useCheckout`'s internal-vs-external branch, and the two copies will drift as the checkout contract evolves (P4 duplication; agent-fixable — route the restart branch through `useCheckout('restore')`).
