# Step 6 — A2 Revision & A6 Foundation — Architect Package (v2)

Second iteration. The first A2 draft is in. This package contains the surgical instructions for the A2 revision pass plus the locked foundation for the A6 build, alongside the source files needed to action both.

## What's new since the first package

- The **A2 first draft** has been reviewed. Most of the foundation is sound; specific alignment fixes are spelled out in the instructions doc below.
- A **family taxonomy** has been identified and locked: intimate screens (A2, A4, A6, A7) use stone + italic prompt; the selection screen (A3) uses eyebrow + upright title. This resolves the largest visual-coherence question.
- The original brief's **9 open questions for A6** have been resolved (11 locked, 2 leans-with-latitude, 1 fully open). You start A6 with a clear floor.

## Package contents (in order)

1. **Revision instructions (primary)** — `Step6_A2_Edits_And_A6_Foundation.md`. The authoritative deliverable for this iteration. Supersedes specific open questions in the original brief where overlap exists.
2. **Original design brief** — `Step6_A2_A6_Design_Brief.md`. Included for context. Where it lists "open questions" that the revision instructions has answered, the revision instructions wins.
3. **Inventory excerpts** — locked A2 and A6 screen rules from the canonical screen inventory.
4. **Current A2 draft** — `essence-step6-a2.html` as it stands. This is the file you'll be editing.
5. **Reference prototypes** — all five existing Step 6 prototypes for mirroring tokens and patterns:
   - `pass2-c-screens.html` — primary architecture reference (dev-rail pattern)
   - `essence-step6-a3.html` — selection family + `.selectable-card` canonical source
   - `essence-step6-a4.html` — intimate family + Ready stone tokens
   - `essence-step6-a5.html` — Working stone reference (for A6 regen transition)
   - `essence-step6-a7.html` — ceremonial close (the register A6 builds toward)

## What we expect back

- A **revised** `essence-step6-a2.html` reflecting all locked changes in Part 1 of the instructions.
- A **new** `essence-step6-a6.html` reflecting Part 2 of the instructions plus your design judgment on the open and lean items.
- Each file includes the inline comment block documenting positioning, tone, and resolutions for any remaining `DESIGN OPEN` items.

---

# 1. Revision instructions — A2 edits + A6 foundation (PRIMARY)
...
# 2. Original design brief (CONTEXT — superseded where the instructions above resolve open questions)
...
# 3. Inventory excerpts — A2 and A6 (locked screen rules)

From `prototypes/message creation/ESSENCE_Step6_Message_Creation_Screen_Inventory.md`. These cannot change without an explicit decision memo.
...
# 4. Current A2 draft — the file you'll be editing
...
<title>ESSENCE · Step 6 · A2 Recipient Setup</title>
...
<!--
  ╔══════════════════════════════════════════════════════════════════════╗
  ║  ESSENCE · STEP 6 · A2 — RECIPIENT SETUP                              ║
  ║  Design memo for the production code architect.                       ║
  ╠══════════════════════════════════════════════════════════════════════╣
  ║                                                                        ║
  ║  POSITIONING INTENT                                                    ║
  ║  A2 is the front door of message creation, not a form to clear. The   ║
  ║  product is voice insurance; the message is a nice thing you can do    ║
  ║  with a voice that is already preserved. So this screen is the first   ║
  ║  warm beat: a quiet "who is this for," set by the stone, before any    ║
  ║  input. Calm, unhurried, never a checkout step.                       ║
  ║                                                                        ║
  ║  TONE TARGETS                                                          ║
  ║  Warm, low-pressure, ceremonial-lite. One soft trust line framed as   ║
  ║  belonging, not defense. No urgency, no emoji, no celebratory marks.   ║
  ║                                                                        ║
  ║  OPEN QUESTION RESOLUTIONS                                             ║
  ║  Q1 Picker shape — TWO surfaces, each on the right canon.             ║
  ║     • Existing-recipient list (A2.b) → avatar selectable-card, the    ║
  ║       pattern a3 explicitly says it "adapts from A2 ... echoes         ║
  ║       recipient avatars." Cards earn their weight: each is a real      ║
  ║       person.                                                          ║
  ║     • Relationship picker (the 7 fixed labels) → chips, NOT cards.     ║
  ║       Reasoning: relationship is a tight, descriptor-less taxonomy.    ║
  ║       Seven full-width cards would swamp the viewport and push the     ║
  ║       name field into scroll, overstating each option. Chips read as   ║
  ║       quick selection, sit on the same tokens, keep 44px+ targets for  ║
  ║       the 45-70 audience, and let the name field stay the primary      ║
  ║       input. The brief invites this differentiation explicitly.        ║
  ║  Q2 a vs b layout — ONE architecture. Recipient list is an optional    ║
  ║     slot above the form. First-ever (a) = pure form. Returning (b) =   ║
  ║     list + Add new, form hidden. Add-new (c) = form revealed, reached  ║
  ║     from the list with a back-to-list path.                           ║
  ║  Q3 List cap — show all (max 3 lifetime). No pagination. Cards stack;  ║
  ║     Add-new ghost card sits at the bottom. Works at 1 / 2 / 3.         ║
  ║  Q4 Validation — Continue disabled until valid; no inline errors.      ║
  ║     Mirrors a3 / onboarding .btn:disabled. (a/c: name + relationship.  ║
  ║     b: a selected recipient.)                                          ║
  ║  Q5 Privacy reassurance — ONE quiet line reframed as PURPOSE, not       ║
  ║     defense: "This only helps shape this message." Per the ESSENCE      ║
  ║     spec, message creation must NOT use Vault language (Voice Vault     ║
  ║     has reserved usage); and any reassurance at first name-entry can    ║
  ║     plant doubt. The line now explains why the field is asked rather    ║
  ║     than promising security. Still a copy-pass candidate — see          ║
  ║     DESIGN OPEN.                                                        ║
  ║  Q6 Stone — centered above the question, --stone-md (140px), a step    ║
  ║     up from a4's companion-scale stone (--stone-sm 100px). a4's stone  ║
  ║     recedes because the textarea is the star; on A2 the stone IS the   ║
  ║     tone-setter with no competing input. Ready state throughout,       ║
  ║     staged entrance. Not hero-scale.                                  ║
  ║  "Someone else" — selecting it reveals one OPTIONAL descriptor field   ║
  ║     ("How would you describe them?"). Optional, never gates Continue.  ║
  ║     Captured to sharpen downstream generation when relationship = other.║
  ║                                                                        ║
  ║  CONTRACT NOTES (carried from Step6_OpenContracts.md + product model)  ║
  ║  • Recipient is NOT promoted to a recipients row at A2 submit, only    ║
  ║    at Save (Q1). Copy never promises "saved to your recipients," and   ║
  ║    headlines avoid "add" language that implies persistence pre-Save.   ║
  ║  • Relationship is MESSAGE-CONTEXT metadata, not global profile data.  ║
  ║    A Message links to both a VoiceProfile and a Recipient, and is      ║
  ║    immutable. The same recipient can hold different relationships      ║
  ║    across voice profiles (Sarah is "daughter" to Mom's voice, could    ║
  ║    be "sister" to another). So: default from the saved recipient when  ║
  ║    one is picked, allow override, persist the chosen relationship onto  ║
  ║    the immutable message. Do NOT treat it as permanent on the          ║
  ║    recipient across profiles.                                          ║
  ║  • Pre-generation state is ephemeral client state; no generationId     ║
  ║    exists yet, no content in URL (Q7).                                 ║
  ║  • Telemetry to wire later: step6.flow_started (entry),                ║
  ║    step6.recipient_selected (submit: recipient_id, relationship,       ║
  ║    is_new_recipient). relationship is denormalized onto save, so this  ║
  ║    screen produces a clean single choice.                             ║
  ╚══════════════════════════════════════════════════════════════════════╝
-->

<style>
:root {
  /* Background ramp (design-tokens.md, canonical) */
...
/* ══════════════════════════════════════════════════════════════════════
   DEV RAIL — prototype-only variation switcher (mirrors pass2-c-screens)
   ══════════════════════════════════════════════════════════════════════ */
...
.dev-rail__nav { display: flex; gap: 6px; flex-wrap: wrap; justify-content: center; max-width: 480px; }
...
.dev-rail__nav button:hover { background: rgba(255,255,255,0.12); color: rgba(255,255,255,0.95); }
.dev-rail__nav button.is-current { background: var(--color-mineral); color: #fff; border-color: var(--color-mineral); }
.dev-rail__nav button.replay { font-size: 11px; }
...
/* The `hidden` attribute is display:none at UA priority, which loses to
   .phone { display:flex }. Force it. */
...
/* ══════════════════════════════════════════════════════════════════════
   BACKBAR — back affordance + step pips (recipient · category · note)
   ══════════════════════════════════════════════════════════════════════ */
.backbar {
...
/* ═══ ENTRANCE CHOREOGRAPHY ═══
   Stone → question → form/list, staggered. Each fades in + drifts up 8px. */
...
/* ══ READY STONE — lifted verbatim from a4, the canonical Ready treatment ══ */
.stone {
...
/* The question — emotional anchor, mirrors a4's prompt-question */
.prompt-question {
...
/* Shared content shell below the question (full width, left-aligned) */
.stage-content {
  width: 100%;
...
/* ══════════════════════════════════════════════════════════════════════
   FORM — name field + relationship chips (A2.a, A2.c)
   ══════════════════════════════════════════════════════════════════════ */
.field-group { display: flex; flex-direction: column; gap: var(--space-md); }
.field-label {
...
/* Name input — single line, adapts a4's note-field treatment */
.name-field {
  width: 100%;
...
.name-field::placeholder { color: var(--color-text-tertiary); font-style: italic; }
.name-field:focus {
...
/* Relationship chips — tight taxonomy, differentiated from cards.
   Rest fill steps clearly lighter than the warm-phase page and carries an
   always-visible mineral hairline, so the tap target reads even before color
   registers (45-70 legibility). Selected steps to a warm gold fill + a
   heavier mineral ring — an unmistakable change from rest. */
.chip-grid { display: flex; flex-wrap: wrap; gap: var(--space-sm); }
.chip {
  min-height: 44px;
  padding: var(--space-sm) var(--space-lg);
  background: var(--color-bg-neutral);
  border: 1.5px solid rgba(122,128,136,0.35);
...
.chip:hover { background: var(--color-bg-warm-1); border-color: var(--color-mineral); }
.chip:active { transform: scale(var(--scale-press-subtle)); }
.chip.is-selected {
...
/* Optional descriptor — collapsed until "Someone else" is chosen */
.describe-group {
  overflow: hidden;
  max-height: 0; opacity: 0; margin-top: 0;
  display: flex; flex-direction: column; gap: var(--space-sm);
  transition:
    max-height var(--duration-medium) var(--ease-essence),
    opacity var(--duration-small) var(--ease-essence),
    margin-top var(--duration-medium) var(--ease-essence);
}
.describe-group.is-open { max-height: 160px; opacity: 1; margin-top: var(--space-md); }

/* Trust line — quiet, belonging not defense (cut candidate, see DESIGN OPEN) */
.trust-line {
  font-family: var(--font-body);
  font-size: var(--text-body);
  color: var(--color-text-secondary);
...
/* ══════════════════════════════════════════════════════════════════════
   RECIPIENT LIST — returning user (A2.b). Avatar selectable-card, the
   pattern a3 echoes. Max 3 lifetime, so no pagination.
   ══════════════════════════════════════════════════════════════════════ */
.recipient-list { display: flex; flex-direction: column; gap: var(--space-md); }
...
.recipient-card:hover { background: var(--color-bg-warm-2); }
.recipient-card:active { transform: scale(var(--scale-press-subtle)); }
...
.recipient-card__avatar {
  ...
  font-family: var(--font-display); font-size: var(--text-body-lg); font-weight: 600;
  transition: background var(--duration-small) var(--ease-essence), color var(--duration-small) var(--ease-essence);
}
.recipient-card.is-selected .recipient-card__avatar {
  background: var(--color-mineral); color: var(--color-bg-neutral);
}
.recipient-card__main { flex: 1; min-width: 0; }
...
.recipient-card__rel {
  ...
  font-size: var(--text-descriptor); color: var(--color-text-secondary);
  margin-top: var(--space-xs); line-height: 1.45;
}
.recipient-card__check { width: 22px; height: 22px; flex-shrink: 0; color: var(--color-mineral); display: none; }
.recipient-card.is-selected .recipient-card__check { display: block; }

/* Add-new — quiet secondary card, clearly not a person. Solid mineral
   hairline (no dashed edges anywhere else in the app), transparent fill so
   it reads lighter than the real recipients above it. */
.recipient-add {
  background: transparent;
  border: 1.5px solid rgba(122,128,136,0.30);
  border-radius: var(--radius-2xl);
  padding: var(--space-lg);
  display: flex; align-items: center; gap: var(--space-lg);
  cursor: pointer; width: 100%; text-align: left;
...
.recipient-add:hover { background: var(--color-bg-warm-2); border-color: var(--color-mineral); }
.recipient-add:active { transform: scale(var(--scale-press-subtle)); }
.recipient-add__icon {
  width: var(--size-avatar); height: var(--size-avatar); flex-shrink: 0;
  border-radius: var(--radius-full);
  border: 1.5px solid rgba(122,128,136,0.35);
  display: flex; align-items: center; justify-content: center;
  color: var(--color-mineral);
}
.recipient-add__label {
...
<div class="dev-rail">
  <div class="dev-rail__label">Step 6 · A2 · Recipient Setup</div>
  <div class="dev-rail__nav" id="devNav">
    <button data-screen="a2a" class="is-current">A2.a · First recipient</button>
    <button data-screen="a2b">A2.b · Returning</button>
    <button data-screen="a2c">A2.c · Add new</button>
    <button data-action="replay" class="replay">↻ Replay</button>
  </div>
</div>

<div class="prototype-wrapper">
  <!-- All three variations share one phone frame; only one is visible at a time. -->

  <!-- ═══════════════════════════════════════════════════════════════════
       A2.a — FIRST-EVER RECIPIENT (pure form, no list)
       ═══════════════════════════════════════════════════════════════════ -->
  <div class="phone" data-screen="a2a">
    ...
    <div class="variant-label">A2.a · First recipient</div>

    <div class="phone__screen">
      <div class="backbar">
        <button class="backbar__btn" data-goto="back" aria-label="Back">
        ...
        <span class="backbar__spacer"></span>
      </div>

      <div class="body">
        <div class="stage">
          <div class="stone" aria-hidden="true"><div class="stone__body"></div></div>
          <h1 class="prompt-question">Who's this first message for?</h1>

          <div class="stage-content">
            <div class="field-group">
              <label class="field-label" for="nameA">Their name</label>
              <input class="name-field" id="nameA" type="text" autocomplete="off"
                     placeholder="Just a first name is fine." aria-label="Recipient name">
            </div>

            <div class="field-group">
              <span class="field-label">Their relationship to you</span>
              <div class="chip-grid" role="group" aria-label="Relationship">
                <button class="chip" type="button" data-rel="daughter" aria-pressed="false">Daughter</button>
                <button class="chip" type="button" data-rel="son" aria-pressed="false">Son</button>
                <button class="chip" type="button" data-rel="partner" aria-pressed="false">Spouse / Partner</button>
                <button class="chip" type="button" data-rel="parent" aria-pressed="false">Parent</button>
                <button class="chip" type="button" data-rel="grandchild" aria-pressed="false">Grandchild</button>
                <button class="chip" type="button" data-rel="friend" aria-pressed="false">Friend</button>
                <button class="chip" type="button" data-rel="other" aria-pressed="false">Someone else</button>
              </div>

              <!-- Optional descriptor — revealed ONLY when "Someone else" is chosen.
                   Optional: does NOT gate Continue. Sharpens downstream generation. -->
              <div class="describe-group" data-describe>
                <label class="field-label" for="describeA">How would you describe them?</label>
                <input class="name-field" id="describeA" type="text" autocomplete="off"
                       placeholder="Neighbor, cousin, caregiver…" aria-label="How would you describe them?">
              </div>
            </div>

            <p class="trust-line">This only helps shape this message.</p>
          </div>
        </div>
      </div>
  ...
  <!-- ═══════════════════════════════════════════════════════════════════
       A2.b — RETURNING USER (existing recipients + Add new)
       Sample shows 2 recipients; production renders 1-3. See DESIGN OPEN.
       ═══════════════════════════════════════════════════════════════════ -->
  <div class="phone" data-screen="a2b" hidden>
    <div class="phone__status">
      <span>9:41</span>
      <div class="phone__status-icons">
        <svg width="16" height="11" viewBox="0 0 16 11" fill="currentColor"><path d="M1 7l2-1 2 2 2-4 2 3 2-1v5H1z"/></svg>
    ...
    <div class="phone__island"></div>
    <div class="phone__home-indicator"></div>
    <div class="variant-label">A2.b · Returning</div>
    ...
    <div class="phone__screen">
      ...
      <div class="body">
        <div class="stage">
          <div class="stone" aria-hidden="true"><div class="stone__body"></div></div>
          <h1 class="prompt-question">Who's this one for?</h1>

          <div class="stage-content">
            <!-- DUPLICATE-NAME EDGE CASE: the two Sarahs share name AND
                 relationship, so each sub line appends a quiet disambiguator
                 ("· last message ..."). Mateo is unique, so no disambiguator.
                 Rule: append ONLY on a name+relationship collision. The
                 disambiguator source (last message category, or date added)
                 is a backend choice — see DESIGN OPEN. Demonstrated here at
                 the 3-recipient lifetime cap. -->
            <div class="recipient-list">
              <button class="recipient-card" type="button" data-recipient="sarah-1">
                <span class="recipient-card__avatar">S</span>
                <span class="recipient-card__main">
                  <span class="recipient-card__name">Sarah</span>
                  <span class="recipient-card__rel">Your daughter · last message, Birthday</span>
                </span>
                <svg class="recipient-card__check" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
              </button>

              <button class="recipient-card" type="button" data-recipient="sarah-2">
                <span class="recipient-card__avatar">S</span>
                <span class="recipient-card__main">
                  <span class="recipient-card__name">Sarah</span>
                  <span class="recipient-card__rel">Your daughter · last message, Encouragement</span>
                </span>
                <svg class="recipient-card__check" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
              </button>

              <button class="recipient-card" type="button" data-recipient="mateo">
                <span class="recipient-card__avatar">M</span>
                <span class="recipient-card__main">
                  <span class="recipient-card__name">Mateo</span>
                  <span class="recipient-card__rel">Your son</span>
                </span>
                <svg class="recipient-card__check" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
              </button>

              <button class="recipient-add" type="button" data-goto="a2c">
                <span class="recipient-add__icon">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                </span>
                <span class="recipient-add__label">Add someone new</span>
              </button>
            </div>
          </div>
        </div>
  ...
  <!-- ═══════════════════════════════════════════════════════════════════
       A2.c — ADDING A NEW RECIPIENT (form, reached from A2.b)
       Back returns to the list.
       ═══════════════════════════════════════════════════════════════════ -->
  <div class="phone" data-screen="a2c" hidden>
    <div class="phone__status">
      <span>9:41</span>
      <div class="phone__status-icons">
        <svg width="16" height="11" viewBox="0 0 16 11" fill="currentColor"><path d="M1 7l2-1 2 2 2-4 2 3 2-1v5H1z"/></svg>
    ...
    <div class="phone__island"></div>
    <div class="phone__home-indicator"></div>
    <div class="variant-label">A2.c · Add new</div>

    <div class="phone__screen">
      <div class="backbar">
        <button class="backbar__btn" data-goto="back" aria-label="Back to your people">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
        </button>
        <div class="backbar__pips">
          <span class="backbar__pip is-current"></span>
      ...
      <div class="body">
        <div class="stage">
          <div class="stone" aria-hidden="true"><div class="stone__body"></div></div>
          <h1 class="prompt-question">Who is this message for?</h1>

          <div class="stage-content">
            <div class="field-group">
              <label class="field-label" for="nameC">Their name</label>
              <input class="name-field" id="nameC" type="text" autocomplete="off"
                     placeholder="Just a first name is fine." aria-label="Recipient name">
            </div>
            ...
            <div class="field-group">
              ...
              <!-- Optional descriptor — revealed ONLY when "Someone else" is chosen. -->
              <div class="describe-group" data-describe>
                <label class="field-label" for="describeC">How would you describe them?</label>
                <input class="name-field" id="describeC" type="text" autocomplete="off"
                       placeholder="Neighbor, cousin, caregiver…" aria-label="How would you describe them?">
              </div>
            </div>
...
<!-- DESIGN OPEN: Backbar pip model assumes 3 input-step pips
     (recipient · category · note) with the back chevron exiting the flow.
     Confirm against a3's actual pip count and back target, since a3 was not
     observed rendering a backbar in its body markup. -->
<!-- DESIGN OPEN: Trust line now reads "This only helps shape this message"
     (purpose framing, no Vault language per spec). Final wording, and whether
     the line survives at all, belong to the copy / legal-trust pass. -->
<!-- DESIGN OPEN: Duplicate-name case is demonstrated in A2.b (two
     Sarah / daughter cards). Disambiguator appends to the sub line ONLY on a
     name+relationship collision. Source field for the disambiguator (last
     message category shown here, or date added) is a backend choice. -->
<!-- DESIGN OPEN: A2.b sample shows the 3-recipient lifetime cap. Layout
     stacks cleanly at 1 / 2 / 3; no pagination needed. -->

<script>
/**
 * Routing in production (Next.js) — see Step6_OpenContracts.md Q7:
 *   /messages/new holds A2 / A3 / A4 in React step state (no URL change).
 *   Continue → A3 (category). Back chevron exits the flow.
 *   On A2.b, selecting an existing recipient stores recipient_id in the
 *   pending_generations row. Add new (A2.c) stores pending_recipient_name +
 *   pending_recipient_relationship — NO recipients row until Save (Q1).
 *
 * The dev rail and history stack below are prototype-only.
 */

// Dev-rail buttons queried once; phone frames queried FRESH each call,
// because the clone-to-replay trick detaches old references.
const navButtons = document.querySelectorAll('#devNav button[data-screen]');
const screenHistory = ['a2a'];
...
function showScreen(name, pushHistory = true) {
  ...
  navButtons.forEach(b => b.classList.toggle('is-current', b.dataset.screen === name));

  // Replay entrance animations on the newly shown screen
...
function goBack() {
  if (screenHistory.length > 1) {
    ...
    // Root — in production this exits to Home. Here, stay on A2.a.
    showScreen('a2a', false);
  }
}

function wireScreen(root) {
  // Routing buttons (back, add-new)
  ...
  // Relationship chips — single select
  const chips = root.querySelectorAll('.chip');
  const describe = root.querySelector('[data-describe]');
  chips.forEach(chip => {
    chip.addEventListener('click', () => {
      chips.forEach(c => { c.classList.remove('is-selected'); c.setAttribute('aria-pressed', 'false'); });
      chip.classList.add('is-selected');
      chip.setAttribute('aria-pressed', 'true');
      // Optional descriptor appears only for "Someone else"
      if (describe) {
        const isOther = chip.dataset.rel === 'other';
        describe.classList.toggle('is-open', isOther);
        if (!isOther) {
          const di = describe.querySelector('input');
          if (di) di.value = '';
        }
      }
      revalidate(root);
  ...
  if (name) name.addEventListener('input', () => revalidate(root));

  // Recipient cards (A2.b) — single select
  const cards = root.querySelectorAll('.recipient-card');
  cards.forEach(card => {
...
function revalidate(root) {
  const btn = root.querySelector('.footer .btn');
  if (!btn) return;
  let valid;
  if (root.querySelector('.form, .field-group')) {
    // A2.a / A2.c — name + relationship both required
    const name = root.querySelector('.name-field');
    const chosenChip = root.querySelector('.chip.is-selected');
    valid = !!(name && name.value.trim() && chosenChip);
  } else {
    // A2.b — a recipient must be selected
    valid = !!root.querySelector('.recipient-card.is-selected');
  }
  btn.disabled = !valid;
}
...
document.querySelectorAll('#devNav button[data-action="replay"]').forEach(b => {
  b.addEventListener('click', () => {
    const current = document.querySelector('#devNav button.is-current');
    showScreen(current ? current.dataset.screen : 'a2a');
  });
});

// Wire the initially visible screen
wireScreen(document.querySelector('.phone[data-screen="a2a"]'));
</script>
</body>
</html>
```
...
# 5. Reference prototype: pass2-c-screens.html

**Primary architecture reference.** Multi-variation prototype with `dev-rail` switching. Mirror this structure for the A6 4-variation rail.
...
# 6. Reference prototype: essence-step6-a3.html

Canonical source for `.selectable-card`, `.selectable-card__icon`, `.selectable-card__main`, `.selectable-card__name`, `.selectable-card__sub`, `.selectable-card__check`. **A2's `.recipient-card` should be renamed to reuse this exact CSS.** Also the canonical `backbar__pip.is-done` style for A6's three completed pips.
...
# 7. Reference prototype: essence-step6-a4.html

Canonical source for the intimate-family stone treatment: `stoneBreath` keyframes, halo geometry, italic `.prompt-question` typography. A2 already lifted this pattern (correctly). A6 will too, with the playback-state extensions specified in the instructions.
...
# 8. Reference prototype: essence-step6-a5.html

Reference for the **Working** stone state — used during A6's regenerate transition per the instructions (A6.4).
...
# 9. Reference prototype: essence-step6-a7.html

Ceremonial close — the emotional register A6 builds toward. Reference for the *quietly-emotional* tone A6 should hit, and for any stone-state continuity between A6 (Playback) and A7 (Infused/Celebrate).
